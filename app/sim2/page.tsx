...
export default function Sim2Page() {
  ...
  /**
   * [v1.20] 역산 목표 개선
   * - 원천징수 15% + 종합소득세 모두 반영한 세후 목표 역산
   * - 이진탐색으로 최종 연도 세후 월배당이 목표에 수렴하도록 계산
   */
  function calcRequired(): number {
    const targetAfterTaxMonthly = targetDiv * 10000 // 세후 목표 (원)
    ...
    for (let i = 0; i < 80; i++) {
      ...
      // 세금 활성화 시 세후 월배당, 비활성 시 세전 월배당
      const monthly = tax.enabled
        ? last?.tax.afterTaxMonthlyDivKRW ?? 0
        : last?.monthlyDivKRW ?? 0
      if (monthly < targetAfterTaxMonthly) lo = mid
      else hi = mid
    }
              ...
              <div className="card p-5 space-y-5">
                ...
                {tax.enabled && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
                    <div className="text-xs text-blue-700 font-semibold mb-1">📊 세후 역산 적용 중</div>
                    <div className="text-xs text-blue-600 leading-relaxed">
                      원천징수 15%{tax.comprehensiveIncomeTax ? ' + 종합소득세' : ''}까지 반영한<br />
                      세후 기준으로 필요 투자금을 역산합니다
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="card p-6">
              ...
              {last && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <ResultCard label={`${years}년 후 포트폴리오`} value={fmtKRW(last.portfolioKRW)}
                    sub={scenario.inflationRate > 0 ? `실질 ${fmtKRW(realValue(last.portfolioKRW))}` : undefined} color="blue" />
                  <ResultCard label="월 배당금" value={fmtKRW(afterTaxMonthly)}
                    sub={tax.enabled ? '세후' : '세전'} color="amber" />
                  <ResultCard label="수익률" value={`+${last.gainPct.toFixed(1)}%`} color="green" />
                  <ResultCard label="총 투자원금" value={fmtKRW(last.invested)} />
                </div>
              )}
            </div>
